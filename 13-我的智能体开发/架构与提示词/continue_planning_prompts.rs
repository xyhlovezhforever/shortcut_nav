...
【关键约束 - 严格执行】
- ⚠️⚠️⚠️ **最高优先级**：所有 tool_id 必须从【可用工具列表】中精确选择，ID完全匹配
- ❌ **严禁**使用可用工具列表之外的任何工具
- ❌ **严禁**臆造、假设、或建议不存在的工具（如 user_input_request、input_collector 等）
- ❌ **严禁**使用已被筛选掉的工具（即使之前版本可能存在）
- ✅ **如果当前可用工具无法完成任务**，必须在 description 字段明确说明："当前可用工具无法完成该任务。建议：1) 提醒管理员添加所需工具（具体说明需要什么类型的工具），或 2) 终止任务"
...
【输出JSON Schema】
{
  ...
  "context_understanding": "简明总结之前的执行情况（2-3句话，包括：已完成的步骤数/名称、执行结果、当前进度状态）",
  "continuation_strategy": "说明接下来的规划思路（2-3句话，包括：为什么这样规划、关键策略、预期目标）",
  "total_steps": 数字,
...
【重要说明 - 必读】
- **context_understanding**（之前执行总结）：用2-3句话简明扼要地总结之前完成了什么，当前处于什么状态
  示例："已完成工具方案文档生成（step_1），成功创建了Qwen和Claude两个架构方案。当前已有完整的方案文档，准备进入代码生成阶段。"

- **continuation_strategy**（接下来的思考）：用2-3句话说明接下来要做什么，为什么这样规划，预期达到什么目标
  示例："接下来将基于用户选择的方案生成完整项目代码。考虑到工具的完整性，将分步生成核心代码、配置文件和测试用例，确保生成的工具可以直接运行。"
...
⚠️ 重要提醒：
1. ⚠️⚠️⚠️ **最重要**：所有 tool_id 必须从上述【可用工具列表】中选择，严禁使用不存在的工具
...
5. **分析上下文中最后一个任务的执行状态**
6. **不要重复执行已成功的步骤**
7. **新步骤的step_id从上下文最后步骤之后继续编号**
8. **可以引用已有步骤的输出** (使用{{step_id.output}}格式)
9. **保持任务逻辑的连贯性和一致性**
10. 如果上方有场景专属指导，必须严格遵守
11. 合理利用并行执行优化性能
12. 多个相同类型操作优先使用actions格式
