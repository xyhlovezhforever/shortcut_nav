...
⚠️ 重要提醒：
1. ⚠️⚠️⚠️ **最重要**：所有 tool_id 必须从上述可用工具列表中选择，严禁使用不存在的工具
2. ❌ **严禁**在计划中臆造工具、假设工具存在、或建议添加新工具
3. ✅ **如果可用工具无法完成任务**：在 description 中说明"当前工具库无法完成任务，建议提醒管理员添加[具体类型]工具或终止任务"
4. 确保每个步骤的 step_name 都有明确的、有意义的名称
5. 避免导致失败的同样问题
6. 如果有标准流程提示，可以参考但不必完全遵循
7. 合理利用并行执行优化性能
8. **充分利用已成功的步骤信息，避免重复执行已成功的步骤**
9. **如果已成功的步骤可以复用，请在新计划中直接引用其输出**
10. **🔴 必须采纳整体反思指导中的改进建议，特别是重新规划策略建议**
