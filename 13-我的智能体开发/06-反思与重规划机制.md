# 06 - 反思与重规划机制

> **文档版本**: v1.2
> **创建日期**: 2026-01-05
> **更新日期**: 2026-01-14
> **迁移优先级**: ⭐⭐⭐⭐ (高)

---

## 1. 机制概述

反思与重规划机制是任务编排服务的自愈能力核心，实现了多层次的智能错误处理和自动恢复。

### 1.1 核心能力

- **失败检测**: 实时检测步骤执行失败
- **原因分析**: LLM分析失败根本原因
- **分层恢复**: 从简单到复杂的恢复策略
- **智能重规划**: 基于反思结果生成新计划
- **🆕 智能轮数管理**: 由大模型决定何时停止反思（v1.1 新增）
- **🆕 单步反思触发整体反思**: 单步反思不再直接停止任务，而是触发整体反思决策（v1.2 新增）
- **🆕 细粒度重规划策略**: 整体反思可建议具体的重规划策略（v1.2 新增）

---

## 2. 三层恢复策略

### 2.1 策略层次图

```
┌─────────────────────────────────────────────────────────────────────┐
│                        三层恢复策略                                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 第1层: 步骤级重试 (Step-Level Retry)                         │   │
│  │                                                             │   │
│  │ 触发条件: 步骤执行失败                                       │   │
│  │ 重试上限: 3次 (可配置)                                       │   │
│  │                                                             │   │
│  │ 重试策略:                                                    │   │
│  │ ├─ 调整参数重试 (RetryWithAdjustedParams)                   │   │
│  │ │   - 根据反思结果调整参数值                                  │   │
│  │ │   - 适用于: 参数错误、格式错误                              │   │
│  │ │                                                           │   │
│  │ └─ 备选工具重试 (RetryWithAlternativeTool)                  │   │
│  │     - 使用功能相同的备选工具                                  │   │
│  │     - 适用于: 工具不可用、工具错误                            │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                            │                                       │
│                            ▼ 重试失败                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 第2层: 单步修复 (Single-Step Repair)                         │   │
│  │                                                             │   │
│  │ 触发条件: 第1层重试达到上限                                   │   │
│  │ 修复上限: 1次 (可配置)                                       │   │
│  │                                                             │   │
│  │ 修复方式:                                                    │   │
│  │ ├─ 调用 Planner.replan_single_step()                        │   │
│  │ ├─ 重新规划失败的单个步骤                                    │   │
│  │ ├─ 保持其他步骤不变                                         │   │
│  │ └─ 复用原计划中已筛选的工具集                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                            │                                       │
│                            ▼ 修复失败                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 第3层: 任务级重规划 (Task-Level Replanning)                   │   │
│  │                                                             │   │
│  │ 触发条件: 第2层修复达到上限                                   │   │
│  │ 重规划上限: 1次 (可配置)                                     │   │
│  │                                                             │   │
│  │ 重规划方式:                                                  │   │
│  │ ├─ 调用 Planner.replan_task()                               │   │
│  │ ├─ 重新执行两阶段规划 (工具筛选 + 任务分解)                   │   │
│  │ ├─ 生成全新执行计划                                         │   │
│  │ └─ 可以引用之前成功步骤的输出                                │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                            │                                       │
│                            ▼ 重规划失败                            │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ 任务失败                                                     │   │
│  │                                                             │   │
│  │ - 返回详细的失败原因                                         │   │
│  │ - 保存执行历史供分析                                         │   │
│  │ - 发送失败事件通知                                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 失败计数器

> **⚠️ 重要更新（v1.1）**: 轮数限制已移除，所有计数器仅用于统计和大模型参考，不再强制限制执行。

| 计数器 | 作用 | 默认上限 | 配置项 | 状态 |
|--------|------|---------|--------|------|
| `step_retry_count` | 当前步骤重试次数 | ~~3~~ `usize::MAX` | `reflection.max_step_retries` | ⚠️ 已弃用 |
| `single_step_repair_count` | 单步修复次数 | ~~1~~ `usize::MAX` | `reflection.max_single_step_repairs` | ⚠️ 已弃用 |
| `task_replan_count` | 任务重规划次数 | ~~1~~ `usize::MAX` | `reflection.max_task_replanning_attempts` | ⚠️ 已弃用 |
| `consecutive_failures` | 连续失败次数 | ~~3~~ 无限制 | - | ⚠️ 已弃用 |
| `current_round` | 当前反思轮次 | 无限制 | - | ✅ 用于统计 |
| `total_step_retries` | 累计步骤重试 | 无限制 | - | ✅ 用于统计 |
| `total_task_replans` | 累计任务重规划 | 无限制 | - | ✅ 用于统计 |

---

## 3. Reflector (反思器) 实现

### 3.1 核心结构

```rust
pub struct Reflector {
    llm_client: UnifiedLlmClient,
    kafka_logger: Arc<KafkaLogger>,
}
```

### 3.2 单步反思方法

```rust
impl Reflector {
    /// 对单个步骤进行反思分析
    pub async fn reflect_on_step(
        &self,
        step: &PlanStep,
        result: &StepExecutionResult,
        context: &ReflectionContext,
        task_type: Option<&str>,
    ) -> Result<StepReflection> {
        // 1. 获取场景指导
        let scene_guidance = SceneManager::new()
            .get_reflection_guidance(task_type);

        // 2. 构建反思提示词
        let (system_prompt, user_prompt) = ReflectionPromptBuilder::new()
            .build_reflection_prompt(
                &context.task_description,
                step,
                result,
                &context.successful_steps,
                task_type,
            );

        // 3. 调用LLM进行反思
        let llm_response = self.llm_client
            .call(&system_prompt, &user_prompt)
            .await?;

        // 4. 解析反思结果
        let reflection = self.parse_reflection_response(&llm_response)?;

        // 5. 记录日志
        self.kafka_logger.log_reflection(&reflection).await;

        Ok(reflection)
    }

    /// 解析LLM反思响应
    fn parse_reflection_response(&self, response: &str) -> Result<StepReflection> {
        // 提取JSON部分
        let json_str = self.extract_json(response)?;

        // 解析为结构体
        let raw: RawReflectionResponse = serde_json::from_str(&json_str)?;

        // 转换为StepReflection
        Ok(StepReflection {
            root_cause_category: self.parse_category(&raw.root_cause_category),
            root_cause: raw.root_cause,
            is_recoverable: raw.is_recoverable,
            confidence: raw.confidence,
            suggested_action: self.parse_action(&raw.suggested_action),
            adjusted_parameters: raw.adjusted_parameters,
            alternative_tool_id: raw.alternative_tool_id,
            improvement_suggestions: raw.improvement_suggestions,
        })
    }
}
```

### 3.3 反思结果结构

```rust
/// 步骤反思结果
pub struct StepReflection {
    /// 根本原因分类
    pub root_cause_category: RootCauseCategory,

    /// 具体根本原因描述
    pub root_cause: String,

    /// 是否可恢复
    pub is_recoverable: bool,

    /// 分析置信度 (0.0 - 1.0)
    pub confidence: f32,

    /// 建议的后续动作
    pub suggested_action: ReflectionAction,

    /// 调整后的参数 (用于参数调整重试)
    pub adjusted_parameters: Option<HashMap<String, String>>,

    /// 备选工具ID (用于工具替换重试)
    pub alternative_tool_id: Option<String>,

    /// 改进建议列表
    pub improvement_suggestions: Vec<String>,
}

/// 根本原因分类
#[derive(Debug, Clone, PartialEq)]
pub enum RootCauseCategory {
    ParameterError,      // 参数错误
    ToolError,           // 工具错误
    DependencyError,     // 依赖错误
    DecompositionError,  // 分解错误
    ExternalError,       // 外部错误
    Unknown,             // 未知错误
}

/// 反思建议动作
#[derive(Debug, Clone, PartialEq)]
pub enum ReflectionAction {
    RetryWithAdjustedParams,  // 调整参数重试
    RetryWithAlternativeTool, // 使用备选工具
    RepairSingleStep,         // 修复单个步骤
    ReplanTask,               // 重新规划任务
    Abort,                    // 中止任务
}
```

---

## 4. Orchestrator 中的反思流程

### 4.1 步骤级反思处理

```rust
impl Orchestrator {
    /// 处理步骤执行失败
    async fn handle_step_failure(
        &self,
        step: &PlanStep,
        result: &StepExecutionResult,
        context: &mut ExecutionContext,
        counters: &mut FailureCounters,
        available_tools: &[ToolInfo],
        task_type: Option<&str>,
    ) -> Result<StepHandleResult> {
        // 1. 调用反思器分析失败原因
        let reflection = self.reflector
            .reflect_on_step(step, result, &self.build_reflection_context(context), task_type)
            .await?;

        // 2. 根据反思结果决定动作
        match reflection.suggested_action {
            // 调整参数重试
            ReflectionAction::RetryWithAdjustedParams => {
                if counters.step_retry_count < self.max_step_retries {
                    counters.step_retry_count += 1;

                    // 使用调整后的参数重试
                    let retry_result = self.executor
                        .execute_single_step_with_overrides(
                            step,
                            reflection.adjusted_parameters.as_ref(),
                            context,
                            available_tools,
                        )
                        .await?;

                    if retry_result.is_success {
                        // 同步输出到metadata
                        self.sync_step_output_to_metadata(
                            &retry_result, context, available_tools
                        );
                        return Ok(StepHandleResult::Success(retry_result));
                    }
                }

                // 重试失败，尝试单步修复
                self.try_single_step_repair(step, result, context, counters, available_tools, task_type).await
            }

            // 备选工具重试
            ReflectionAction::RetryWithAlternativeTool => {
                if counters.step_retry_count < self.max_step_retries {
                    if let Some(alt_tool_id) = &reflection.alternative_tool_id {
                        counters.step_retry_count += 1;

                        // 使用备选工具重试
                        let mut alt_step = step.clone();
                        alt_step.tool = alt_tool_id.clone();

                        let retry_result = self.executor
                            .execute_single_step(&alt_step, context, available_tools)
                            .await?;

                        if retry_result.is_success {
                            self.sync_step_output_to_metadata(
                                &retry_result, context, available_tools
                            );
                            return Ok(StepHandleResult::Success(retry_result));
                        }
                    }
                }

                self.try_single_step_repair(step, result, context, counters, available_tools, task_type).await
            }

            // 直接进入单步修复
            ReflectionAction::RepairSingleStep => {
                self.try_single_step_repair(step, result, context, counters, available_tools, task_type).await
            }

            // 直接进入任务重规划
            ReflectionAction::ReplanTask => {
                self.try_task_replan(context, counters, task_type).await
            }

            // 中止任务
            ReflectionAction::Abort => {
                Ok(StepHandleResult::Abort(reflection.root_cause))
            }
        }
    }
}
```

### 4.2 单步修复处理

```rust
impl Orchestrator {
    /// 尝试单步修复
    async fn try_single_step_repair(
        &self,
        failed_step: &PlanStep,
        failed_result: &StepExecutionResult,
        context: &mut ExecutionContext,
        counters: &mut FailureCounters,
        available_tools: &[ToolInfo],
        task_type: Option<&str>,
    ) -> Result<StepHandleResult> {
        // 检查修复次数限制
        if counters.single_step_repair_count >= self.max_single_step_repairs {
            return self.try_task_replan(context, counters, task_type).await;
        }

        counters.single_step_repair_count += 1;

        // 调用Planner进行单步重新规划
        let repaired_step = self.planner
            .replan_single_step(
                &failed_step.step_id,
                failed_step,
                &failed_result.error_message.as_deref().unwrap_or("unknown error"),
                available_tools,  // 复用原工具集
            )
            .await?;

        // 执行修复后的步骤
        let repair_result = self.executor
            .execute_single_step(&repaired_step, context, available_tools)
            .await?;

        if repair_result.is_success {
            self.sync_step_output_to_metadata(&repair_result, context, available_tools);
            Ok(StepHandleResult::Success(repair_result))
        } else {
            // 修复失败，尝试任务重规划
            self.try_task_replan(context, counters, task_type).await
        }
    }
}
```

### 4.3 任务重规划处理

```rust
impl Orchestrator {
    /// 尝试任务级重规划
    async fn try_task_replan(
        &self,
        context: &mut ExecutionContext,
        counters: &mut FailureCounters,
        task_type: Option<&str>,
    ) -> Result<StepHandleResult> {
        // 检查重规划次数限制
        if counters.task_replan_count >= self.max_task_replanning_attempts {
            return Ok(StepHandleResult::Abort("达到最大重规划次数".to_string()));
        }

        counters.task_replan_count += 1;

        // 构建重规划上下文
        let replanning_context = self.build_replanning_context(context);

        // 调用Planner进行任务重规划
        let new_plan = self.planner
            .replan_task(
                &replanning_context.failure_summary,
                replanning_context.metadata,
            )
            .await?;

        // 注意: 不清除ExecutionContext，保留已成功步骤的输出
        // context.clear() 方法已修改为不清除数据

        Ok(StepHandleResult::Replan(new_plan))
    }
}
```

---

## 5. 单步修复 vs 任务重规划

### 5.1 对比表

| 特性 | 单步修复 | 任务重规划 |
|------|---------|-----------|
| **触发条件** | 第1层重试失败 | 单步修复失败 |
| **修改范围** | 只修改失败步骤 | 生成全新计划 |
| **工具筛选** | 不执行，复用原工具集 | 重新执行两阶段规划 |
| **保留数据** | 保留所有历史数据 | 保留所有历史数据 |
| **适用场景** | 步骤设计问题 | 整体规划问题 |
| **开销** | 较小 | 较大 |

### 5.2 决策流程

```
┌─────────────────────────────────────────────────────────────────────┐
│                       单步修复 vs 任务重规划 决策                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  步骤失败                                                            │
│       │                                                             │
│       ▼                                                             │
│  ┌────────────────────────────────────────────┐                    │
│  │ LLM反思分析                                  │                    │
│  │ - 识别根本原因类型                           │                    │
│  │ - 判断是否可恢复                             │                    │
│  └─────────────────┬──────────────────────────┘                    │
│                    │                                                │
│       ┌────────────┴────────────┐                                  │
│       │                         │                                  │
│       ▼                         ▼                                  │
│  ┌─────────────┐          ┌─────────────┐                          │
│  │ 局部问题    │          │ 全局问题    │                          │
│  │             │          │             │                          │
│  │ - 参数错误  │          │ - 步骤缺失  │                          │
│  │ - 工具错误  │          │ - 顺序错误  │                          │
│  │ - 单点失败  │          │ - 依赖断裂  │                          │
│  └──────┬──────┘          └──────┬──────┘                          │
│         │                        │                                  │
│         ▼                        ▼                                  │
│  ┌─────────────┐          ┌─────────────┐                          │
│  │ 单步修复    │          │ 任务重规划  │                          │
│  │             │          │             │                          │
│  │ - 修改失败  │          │ - 重新筛选  │                          │
│  │   步骤      │          │   工具      │                          │
│  │ - 保留其他  │          │ - 生成新    │                          │
│  │   步骤      │          │   计划      │                          │
│  └─────────────┘          └─────────────┘                          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 6. 数据保持机制

### 6.1 重规划时的数据处理

```rust
// ExecutionContext::clear() 的关键设计
pub fn clear(&self) {
    // 重要: 不清除 step_results 和 runtime_metadata
    // 这样重规划后的新步骤可以引用之前成功步骤的输出

    info!(
        "执行上下文重置：保留已有的步骤结果和运行时元数据"
    );

    // 不执行以下操作:
    // self.step_results.write().unwrap().clear();
    // self.runtime_metadata.write().unwrap().clear();
}
```

### 6.2 数据引用示例

```
原计划:
  step_1 (成功) → output: {data_id: "d1"}
  step_2 (成功) → output: {datasource_id: "ds1"}
  step_3 (失败) → error: "处理失败"
  step_4 (未执行)

重规划后:
  step_3_new → input: {{step_2.outputs.datasource_id}}  ✅ 可以引用
  step_4_new → input: {{step_3_new.outputs.result}}
  step_5_new → input: {{step_1.outputs.data_id}}        ✅ 可以引用
```

---

## 7. 配置说明

### 7.1 配置文件

```toml
# config.toml

[reflection]
# 是否启用步骤级反思
enable_step_level_reflection = true

# 每个步骤的最大重试次数
max_step_retries = 3

# 单步修复的最大次数
max_single_step_repairs = 1

# 任务重规划的最大次数
max_task_replanning_attempts = 1

# 连续失败阈值 (达到后强制进入重规划)
consecutive_failure_threshold = 3
```

### 7.2 配置加载

```rust
#[derive(Debug, Clone, Deserialize)]
pub struct ReflectionConfig {
    #[serde(default = "default_enable_reflection")]
    pub enable_step_level_reflection: bool,

    #[serde(default = "default_max_step_retries")]
    pub max_step_retries: u32,

    #[serde(default = "default_max_single_step_repairs")]
    pub max_single_step_repairs: u32,

    #[serde(default = "default_max_task_replanning")]
    pub max_task_replanning_attempts: u32,
}

fn default_enable_reflection() -> bool { true }
fn default_max_step_retries() -> u32 { 3 }
fn default_max_single_step_repairs() -> u32 { 1 }
fn default_max_task_replanning() -> u32 { 1 }
```

---

## 8. 智能轮数管理 (v1.1 新增)

### 8.1 设计理念

从人为限制转向智能决策：

**修改前（v1.0）**:
```
┌─────────────────────────────────────┐
│ 反思循环（人为限制）                  │
├─────────────────────────────────────┤
│ 1. 检查轮次 ≥ max_rounds? → 强制停止 │
│ 2. 检查连续失败 ≥ 3? → 强制停止      │
│ 3. 执行反思 → 获取 should_replan     │
│ 4. should_replan && !达限 → 继续     │
└─────────────────────────────────────┘
```

**修改后（v1.1）**:
```
┌─────────────────────────────────────┐
│ 反思循环（智能决策）                  │
├─────────────────────────────────────┤
│ 1. 执行反思 → 获取 should_replan     │
│ 2. should_replan = true → 继续       │
│ 3. should_replan = false → 停止      │
│ 4. 大模型基于统计信息自主判断        │
└─────────────────────────────────────┘
```

### 8.2 提示词软性引导

为避免大模型无限反思，提示词中加入**软性引导**：

```
📊 轮次参考指南（仅供参考，最终由你决定）：
   • 1-3 轮：正常范围，可根据需要继续优化
   • 4-5 轮：建议审视是否有实质性改进空间
   • 6-10 轮：⚠️ 超出常规范围，请认真评估继续反思的价值
   • 10 轮以上：🚨 强烈建议停止，除非有明确的新解决思路

⚠️ 重要提醒：
   - 是否继续反思完全由你决定，没有强制的轮数限制
   - 但请注意：连续多轮未能解决问题时，继续反思往往只是重复无效尝试
   - 当你发现无法提供新见解时，应果断设置 should_replan = false
```

### 8.3 执行统计摘要

执行历史文件中加入统计摘要，帮助大模型决策：

```markdown
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 执行统计摘要
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  • 当前反思轮次: 第 X 轮
  • 累计步骤重试次数: X 次
  • 累计任务重规划次数: X 次
  • 成功步骤数: X
  • 失败步骤数: X
```

大模型可以根据这些统计信息判断：
- **重试次数过多** → 考虑更换方案
- **重规划次数过多** → 可能存在根本性问题
- **轮次过多但无进展** → 果断停止

### 8.4 配置变更

```rust
// src/config/mod.rs
pub struct ReflectionConfig {
    /// 已弃用，仅保留向后兼容
    #[deprecated]
    pub max_step_retries: usize,  // 默认: usize::MAX

    #[deprecated]
    pub max_task_replanning_attempts: usize,  // 默认: usize::MAX

    /// 是否启用步骤级反思（仍然有效）
    pub enable_step_level_reflection: bool,  // 默认: true
}
```

---

## 9. 迁移检查项

### 9.1 Reflector
- [ ] 实现 `reflect_on_step()` 方法
- [ ] 实现反思提示词构建
- [ ] 实现响应解析逻辑
- [ ] 定义 `StepReflection` 结构
- [ ] 定义 `RootCauseCategory` 枚举
- [ ] 定义 `ReflectionAction` 枚举

### 9.2 Orchestrator反思流程
- [ ] 实现 `handle_step_failure()` 方法
- [ ] 实现参数调整重试逻辑
- [ ] 实现备选工具重试逻辑
- [ ] 实现 `try_single_step_repair()` 方法
- [ ] 实现 `try_task_replan()` 方法
- [ ] ⚠️ 移除失败计数器硬限制（v1.1改动）
- [ ] ✅ 实现统计信息收集（v1.1改动）

### 9.3 Planner重规划
- [ ] 实现 `replan_single_step()` 方法
- [ ] 实现 `replan_task()` 方法
- [ ] 确保单步修复复用原工具集
- [ ] 确保任务重规划执行两阶段流程

### 9.4 智能轮数管理（v1.1新增）
- [ ] ✅ 标记轮数限制配置为deprecated
- [ ] ✅ 移除循环中的轮数检查
- [ ] ✅ 在提示词中加入软性引导
- [ ] ✅ 实现执行统计摘要
- [ ] ✅ 更新ContextEngineeringEventBuilder结构

### 9.5 数据保持
- [ ] 修改 `ExecutionContext::clear()` 保留历史数据
- [ ] 确保重试成功后同步到metadata
- [ ] 确保重规划后可以引用历史输出

---

## 10. 单步反思触发整体反思 (v1.2 新增)

### 10.1 设计变更

**修改前（v1.1）**:
```
单步反思失败 → StopExecution → 任务直接停止
```

**修改后（v1.2）**:
```
单步反思达到限制 → TriggerOverallReflection → 整体反思
                                                    ↓
                                    ┌───────────────┴───────────────┐
                                    ↓                               ↓
                            should_replan=true              should_replan=false
                                    ↓                               ↓
                        重新规划（带策略建议）                   停止任务
```

### 10.2 StepAction 枚举变更

```rust
// src/core/reflector.rs:52-63
pub enum StepAction {
    RetryWithAdjustedParams(HashMap<String, String>),
    RetryWithAlternativeTool(String),
    ReplanEntireTask,
    TriggerOverallReflection(String), // 新增：携带摘要信息
    // StopExecution 已移除
}
```

### 10.3 处理流程

当单步反思返回 `TriggerOverallReflection` 时：

1. 编排器记录触发事件到 Kafka
2. 调用整体反思进行全局分析
3. 根据 `should_replan` 决定：
   - `true`: 触发重新规划（可能带细粒度策略）
   - `false`: 停止任务并返回用户介入错误

---

## 11. 细粒度重规划策略 (v1.2 新增)

### 11.1 ReplanningStrategy 枚举

```rust
// src/llm/replanning_prompts.rs:23-43
pub enum ReplanningStrategy {
    /// 完整重新规划整个任务
    FullReplan,

    /// 从指定步骤开始重新规划
    ReplanFromStep { step_id: String, reason: String },

    /// 跳过指定的步骤
    SkipSteps { step_ids: Vec<String>, reason: String },

    /// 添加补救步骤
    AddRemediationSteps { suggestions: Vec<String> },

    /// 调整步骤间依赖关系
    AdjustDependencies { adjustments: Vec<String> },
}
```

### 11.2 策略说明

| 策略 | 适用场景 |
|------|---------|
| `FullReplan` | 整体方案有根本性问题 |
| `ReplanFromStep` | 前面步骤成功，从失败点重新规划 |
| `SkipSteps` | 某些步骤被证明不必要 |
| `AddRemediationSteps` | 需要添加额外步骤修复问题 |
| `AdjustDependencies` | 步骤依赖关系需要调整 |

### 11.3 整体反思指导信息传递

```rust
// src/llm/replanning_prompts.rs:45-57
pub struct OverallReflectionGuidance {
    pub root_causes: Vec<String>,
    pub incorrect_assumptions: Vec<String>,
    pub alternative_approaches: Vec<String>,
    pub lessons_learned: Vec<String>,
    pub replanning_strategy: Option<ReplanningStrategy>,
}
```

整体反思的指导信息会传递给重新规划阶段，通过 `format_overall_reflection_guidance()` 方法格式化为文本，插入到重新规划提示词中。

### 11.4 代码位置映射

| 功能 | 文件 | 行号 |
|------|------|------|
| StepAction 枚举 | `src/core/reflector.rs` | 52-63 |
| ReplanningStrategy 枚举 | `src/llm/replanning_prompts.rs` | 23-43 |
| OverallReflectionGuidance | `src/llm/replanning_prompts.rs` | 45-57 |
| 策略解析逻辑 | `src/core/reflector.rs` | 648-836 |
| 编排器处理 | `src/core/orchestrator.rs` | 2580-2673 |
| 指导信息传递 | `src/core/orchestrator.rs` | 2947-2958 |

---

## 12. 迁移检查项 (v1.2 更新)

### 12.1 单步反思触发整体反思（v1.2新增）
- [ ] ✅ 移除 `StepAction::StopExecution`
- [ ] ✅ 新增 `StepAction::TriggerOverallReflection`
- [ ] ✅ 更新单步反思提示词（stop → trigger_overall_reflection）
- [ ] ✅ 编排器处理 TriggerOverallReflection 逻辑
- [ ] ✅ grpc/server.rs 更新事件处理

### 12.2 细粒度重规划策略（v1.2新增）
- [ ] ✅ 定义 `ReplanningStrategy` 枚举
- [ ] ✅ 定义 `OverallReflectionGuidance` 结构
- [ ] ✅ ReflectionResult 新增 `replanning_strategy` 字段
- [ ] ✅ 实现策略解析逻辑 `parse_replanning_strategy()`
- [ ] ✅ 整体反思提示词增加策略指导
- [ ] ✅ 实现 `format_overall_reflection_guidance()` 方法
- [ ] ✅ 编排器传递指导信息到重新规划

---

## 下一步

阅读 [07-并行执行机制.md](./07-并行执行机制.md) 了解并行执行的详细实现。
